...
KVER=5.1.1
K="$T/linux-${KVER}"
kernel="${KVER}-CPI-Kernel"
uboot="u-boot-2019.07"
EXTLINUX=chaospi/${BOARD}/linux4/
...
R="${SD}/CPI-ROOT"
	...
	#
	mkdir -p $R/usr/lib/u-boot/chaospi/${uboot}
	cp -a $U/*.gz $R/usr/lib/u-boot/chaospi/${uboot}
	#
	## copy files to BPI-ROOT/boot
	#
	rm -rf $R/boot
	mkdir -p $R/boot
	cp -a $K/output/${BOARD}/arch/${ARCH}/boot/${KERNEL} $R/boot/vmlinuz-5.1.1-cpi
	mkdir -p $R/efi
	cp -a $T/efi/* $R/efi
	#
	...
	# BPI-ROOT: BOOTLOADER
	(cd $R ; tar czvf $SD/BOOTLOADER-${BOARD}-linux4.tgz usr/lib/u-boot/chaospi)
